...
| ID | 内容 | 状態 |
|----|------|------|
| U6-04 | IPC 入口 Zod（主要 `dmig:*` ハンドラ） | 実装中 |
| U6-06 | manifest Zod 一元化（`parseDmigManifestPayload`） | 実装中 |
| U6-07 | `checksums.sha256` 原子的書き込み | 実装中 |
| U6-08 | compose 設定ファイルサイズ上限 | 実装中 |
| U6-09 | alpine ヘルパーイメージ digest 固定 | 実装中 |
