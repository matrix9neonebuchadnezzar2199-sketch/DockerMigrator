> **撤回済み (2026-05-19)**: Step B（ウェルカムウィザード + Settings S2）は実装後に撤回。実装コミット群は `1587fa3`〜`d9794bc`、撤回は本日の revert コミットを参照。再実行用ではなく履歴参照用。

---

# Phase 6 第4回 Step B 指示書: 初回起動ウィザード「移行元 or 移行先？」（撤回）

**対象モデル**: Composer2  
**作業日**: 2026-05-19  
**起点コミット**: `328ddf2`（5/19 日記 pending 解消後）  
**撤回理由**: Step A サイドバー 3 グループで入口が足りウィザードは冗長。実機不具合もあり UX 二重化を解消。Settings 永続化は将来の独立「設定」タスクで再設計。
