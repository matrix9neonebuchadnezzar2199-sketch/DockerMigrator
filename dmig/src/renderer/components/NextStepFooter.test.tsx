...
describe('NextStepFooter', () => {
  ...
  it('source-overview: 説明と CTA、クリックで compose', async () => {
    ...
    render(<NextStepFooter page="source-overview" onNavigate={onNavigate} dockerConnected />);
  ...
  it('compose: CTA で export へ', async () => {
    const user = userEvent.setup();
    const onNavigate = vi.fn();
    render(<NextStepFooter page="compose" onNavigate={onNavigate} dockerConnected />);
  ...
  it('export: 説明のみ、CTA ボタンなし', () => {
    render(<NextStepFooter page="export" onNavigate={vi.fn()} dockerConnected />);
    expect(screen.getByText(/USB/)).toBeInTheDocument();
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });

  it('import: 説明のみ、CTA ボタンなし', () => {
    render(<NextStepFooter page="import" onNavigate={vi.fn()} dockerConnected />);
  ...
  it('target-overview: CTA で import へ', async () => {
    const user = userEvent.setup();
    const onNavigate = vi.fn();
    render(<NextStepFooter page="target-overview" onNavigate={onNavigate} dockerConnected />);
    await user.click(screen.getByRole('button', { name: 'パックを読み込む' }));
    expect(onNavigate).toHaveBeenCalledWith('import');
  });

  it('resume: 説明のみ', () => {
    render(<NextStepFooter page="resume" onNavigate={vi.fn()} dockerConnected />);
    expect(screen.getByText(/中断したパック/)).toBeInTheDocument();
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });

  it('Docker 未接続: 接続案内のみ CTA なし', () => {
    render(<NextStepFooter page="source-overview" onNavigate={vi.fn()} dockerConnected={false} />);
    expect(screen.getByText(/Docker Desktop を起動/)).toBeInTheDocument();
    expect(screen.queryByRole('button')).not.toBeInTheDocument();
  });

  it('help: フッター非表示', () => {
    render(<NextStepFooter page="help" onNavigate={vi.fn()} dockerConnected />);
